...
#     UPGRADE_SCRIPT_VERSION -> An optional variable to override the version of the upgrade script used from
#         p1as-upgrades repo when NEW_VERSION is v1.19+
...
# default set of environments: v1.11.0-dev, v1.11.0-test, v1.11.0-stage, v1.11.0-master, and v1.11.0-customer-hub. These new branches will be
...
#
# NOTE: for v1.18 only this script has been updated to handle both v1.19+ upgrade logic and v1.18 and older
...
if [[ "${NEW_VERSION}" = "v1.19"* ]]; then
  echo "Using new v1.19.*.* upgrade wrapper logic"
  ...
  echo "Using v1.18.*.* and older upgrade wrapper logic"
